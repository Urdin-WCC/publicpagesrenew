...
export default function SearchResultsClient({ searchQuery }: SearchResultsClientProps) {
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <h2 className="text-xl font-semibold mb-4">Introduce un término de búsqueda</h2>
          <p className="text-gray-500">Utiliza el formulario para buscar proyectos.</p>
        </div>
      ) : !data || data.projects.length === 0 ? (
        <div className="text-center py-12">
          <h2 className="text-xl font-semibold mb-4">
            {translations.public.noPortfolioResultsFor.replace('{0}', searchQuery)}
          </h2>
        ...
        <>
          <p className="mb-6 text-gray-600">
            Se encontraron {data.totalProjects} resultados para "{searchQuery}"
          </p>
          ...
          {data.totalPages > 1 && (
            <div className="mt-8 flex justify-center">
              <PaginationControls
                ...
                basePath={`/portfolio/search?q=${encodeURIComponent(searchQuery)}`}
              />
            </div>
          )}
        </>
