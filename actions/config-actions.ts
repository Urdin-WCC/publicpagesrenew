...
import { getGlobalConfig, updateGlobalConfig, GlobalConfigWithCustomFields } from "@/lib/config-server"; // Importar desde el archivo de servidor

/**
 * Server Action para obtener la configuración global.
 */
export async function fetchGlobalConfig(): Promise<GlobalConfigWithCustomFields | null> {
 ...
 */
export async function saveGlobalConfig(data: Partial<GlobalConfigWithCustomFields>) {
  try {
    await updateGlobalConfig(data);
    // Revalidar rutas relevantes si es necesario (ej: layout principal)
    revalidatePath("/");
    revalidatePath("/admin/settings/header"); // Revalidar la propia página
