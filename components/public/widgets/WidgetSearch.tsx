...
export default function WidgetSearch({ title }: WidgetSearchProps) {
  ...
  return (
    <div className="widget p-4 border rounded bg-white shadow-sm mb-4">
      <h3 className="widget-title text-lg font-semibold mb-3">{title || translations.common.search}</h3>
      <form onSubmit={handleSearch} className="flex">
        <input
          type="search"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          placeholder={translations.public.searchPlaceholder || "Escribe tu búsqueda..."}
